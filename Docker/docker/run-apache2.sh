...
if [ -z "$_DECK_CERT_PATH" ]; then
	# SSL not enabled
	cp /opt/deck/bin/spinnaker.conf.gen spinnaker.conf
else
...
cp /opt/deck/bin/httpd.conf.gen /etc/apache2/httpd.conf

# Create a passphrase file to inject the SSL passphrase into apache's startup
cp /opt/deck/bin/passphrase.gen passphrase
...
then
  ...
  echo '[]' > /opt/deck/html/plugin-manifest.json
fi
